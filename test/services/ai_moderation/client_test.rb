...
class AiModeration::ClientTest < ActiveSupport::TestCase
  ...
  test "sets mistral api key when provider is mistral" do
    config = {
      provider: "mistral",
    ...
    RubyLLM.define_singleton_method(:configure) { |_ = nil, &block| block&.call(fake_llm_config) }
    RubyLLM.define_singleton_method(:chat) { |model:, provider:| fake_chat }
    ...
    assert_equal "mistral-test-key", fake_llm_config.mistral_api_key
  end

  test "returns failed decision when provider has no api key" do
    config = {
      provider: "openai",
